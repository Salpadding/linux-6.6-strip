...
PHONY += scripts_basic
scripts_basic:
	@echo $(INDENT) $(MAKE) $(build)=scripts/basic
	$(Q)$(MAKE) INDENT=$(INDENT):$@ $(build)=scripts/basic
	@echo $(INDENT) $(MAKE) $(build)=scripts/basic done
...
%config: outputmakefile scripts_basic FORCE
	@echo $(INDENT) $(MAKE) $(build)=scripts/kconfig $@
	$(Q)$(MAKE) INDENT=$(INDENT):$@ $(build)=scripts/kconfig $@
	@echo $(INDENT) $(MAKE) $(build)=scripts/kconfig $@ done
...
$(info $(INDENT) mixed-build=$(mixed-build))
$(info $(INDENT) need-config=$(need-config))
$(info $(INDENT) may-sync-config=$(may-sync-config))

ifdef need-config
$(info $(INDENT) include include/config/auto.conf)
...
$(clean-dirs):
	@echo $(INDENT) $(MAKE) $(clean)=$(patsubst _clean_%,%,$@)
	$(Q)$(MAKE) INDENT=$(INDENT):$@ $(clean)=$(patsubst _clean_%,%,$@)
